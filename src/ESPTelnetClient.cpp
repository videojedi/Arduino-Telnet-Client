...
ESPtelnetClient::ESPtelnetClient(WiFiClient& client){
	this->client = &client;	
}

bool ESPtelnetClient::login(IPAddress serverIpAddress, const char* username, const char* password, uint8_t port){
...
bool ESPtelnetClient::sendCommand(const char* cmd){
...
void ESPtelnetClient::disconnect(){
	this->client->stop();
}

bool ESPtelnetClient::send(const char* buf, bool waitEcho){
...
void ESPtelnetClient::negotiate(){
...
bool ESPtelnetClient::listenUntil(char c){
...
bool ESPtelnetClient::waitPrompt(){
...
void ESPtelnetClient::print(char c){
	//edit this function if you want a different output!
	Serial.print(c);
}

void ESPtelnetClient::setPromptChar(char c){
	m_promptChar = c;
}
